...
export const Review = () => {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-white/50">Loading Session...</div>;
  ...
  if (cards.length === 0 && !sessionComplete) {
    return (
      <div className="max-w-md mx-auto mt-20 text-center glass-panel p-10">
        ...
        <p className="text-white/60 mb-8">No cards due for review in this deck right now.</p>
        <Link to="/" className="btn-primary px-6 py-3 rounded-xl inline-block">Back to Dashboard</Link>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-md mx-auto mt-20 text-center glass-panel p-10">
        ...
        >
          ...
          <h2 className="text-3xl font-bold mb-2 bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-cyan-400">Session Complete!</h2>
          <p className="text-white/60 mb-8">You've reviewed {cards.length} cards.</p>
          <Link to="/" className="btn-primary px-8 py-3 rounded-xl inline-block font-bold">Back to Dashboard</Link>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto py-10 px-4 flex flex-col h-[calc(100vh-100px)]">
      ...
      <div className="flex-1 flex flex-col justify-center relative perspective-1000">
        ...
        <div className="mt-8 text-center h-24">
          {!isFlipped ? (
            <p className="text-white/40 animate-pulse">Tap card to see answer</p>
          ) : (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <button onClick={() => handleGrade(0)} className="p-3 rounded-xl bg-red-500/20 text-red-300 border border-red-500/30 hover:bg-red-500 hover:text-white transition-all">
                <span className="block text-xs font-bold uppercase opacity-70 mb-1">Again</span>
                <span className="text-sm">Cannot</span>
              </button>
              <button onClick={() => handleGrade(3)} className="p-3 rounded-xl bg-orange-500/20 text-orange-300 border border-orange-500/30 hover:bg-orange-500 hover:text-white transition-all">
                <span className="block text-xs font-bold uppercase opacity-70 mb-1">Hard</span>
                <span className="text-sm">Struggled</span>
              </button>
              <button onClick={() => handleGrade(4)} className="p-3 rounded-xl bg-blue-500/20 text-blue-300 border border-blue-500/30 hover:bg-blue-500 hover:text-white transition-all">
                <span className="block text-xs font-bold uppercase opacity-70 mb-1">Good</span>
                <span className="text-sm">Recall</span>
              </button>
              <button onClick={() => handleGrade(5)} className="p-3 rounded-xl bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 hover:bg-emerald-500 hover:text-white transition-all">
                <span className="block text-xs font-bold uppercase opacity-70 mb-1">Easy</span>
